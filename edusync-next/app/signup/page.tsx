...
import { ArrowLeft, ArrowRight, Plus, X, Check, BookOpen, AlertCircle, Loader2 } from "lucide-react"
...
export default function SignupPage() {
  ...
  const addSubject = () => {
    if (newSubjectName && newSubjectProficiency) {
      setSubjects([
        ...subjects,
        { name: newSubjectName, proficiency: newSubjectProficiency },
      ])
      setNewSubjectName("")
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 1:
        return name && email && password && password === confirmPassword
      case 2:
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/auth/register`, {
        ...
        body: JSON.stringify({
          ...
          learningStyle: {
            visual: selectedLearningStyles.includes("Visual"),
            auditory: selectedLearningStyles.includes("Auditory"),
            kinesthetic: selectedLearningStyles.includes("Kinesthetic"),
          },
        }),
      })
                ...
                >
                  <div
                    className={cn(
                      ...
                      (() => {
                        if (step.id < currentStep) return "bg-primary text-primary-foreground"
                        if (step.id === currentStep) return "border-2 border-primary bg-primary/10 text-primary"
                  ...
                  <span className="hidden text-xs font-medium sm:block mt-2">
                    {step.title}
                  </span>
                </div>
              ))}
              ...
              {currentStep === 2 && (
                <div className="space-y-6">
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={setNewSubjectProficiency}
                        disabled={isSubmitting}
                      >
                        <SelectTrigger>
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={addSubject}
                    disabled={!newSubjectName || !newSubjectProficiency || isSubmitting}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {subjects.map((subject, index) => (
                          <Badge
                            key={`${subject.name}-${subject.proficiency}`}
                            variant="secondary"
              ...
              {currentStep === 3 && (
                <div className="space-y-6">
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={addTimeSlot}
                    disabled={!selectedDay || !startTime || !endTime || isSubmitting}
                    className="w-full gap-2"
                  >
                    <Plus className="h-4 w-4" />
                    Add Time Slot
